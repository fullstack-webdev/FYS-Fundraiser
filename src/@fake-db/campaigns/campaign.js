...
mock.onPost('/apps/campaigns/add-campaign').reply(config => {
  // Get event from post data
  const campaign = JSON.parse(config.data)
  const highestValue = data.campaigns.reduce((a, b) => (a.id > b.id ? a : b)).id

  campaign.id = highestValue + 1
...
mock.onGet('/api/campaigns/list/data').reply(config => {
  ...
  const dataAsc = data.campaigns.sort((a, b) => (a[sortColumn] < b[sortColumn] ? -1 : 1))

  const dataToFilter = sort === 'asc' ? dataAsc : dataAsc.reverse()

  const filteredData = dataToFilter.filter(
    campaign =>
      (campaign.email.toLowerCase().includes(queryLowered) ||
      campaign.fullName.toLowerCase().includes(queryLowered) ||
      campaign.billing.toLowerCase().includes(queryLowered)) &&
      campaign.role === (role || campaign.role) &&
      campaign.currentPlan === (currentPlan || campaign.currentPlan) &&
...
mock.onGet('/api/campaigns/campaign').reply(config => {
  const { id } = config
  const campaign = data.campaigns.find(i => i.id === id)
  return [200, { campaign }]
})

// DELETE: Deletes User
mock.onDelete('/apps/campaigns/delete').reply(config => {
  ...
  const campaignIndex = data.campaigns.findIndex(t => t.id === campaignId)
