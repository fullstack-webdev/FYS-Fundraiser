...
// GET ALL DATA
mock.onGet('/api/fundraisers/list/all-data').reply(200, data.fundraisers)

// POST: Add new fundraiser
mock.onPost('/apps/fundraisers/add-fundraiser').reply(config => {
  // Get event from post data
  const fundraiser = JSON.parse(config.data)
  const highestValue = data.fundraisers.reduce((a, b) => (a.id > b.id ? a : b)).id

  fundraiser.id = highestValue + 1

  data.fundraisers.push(fundraiser)

  return [201, { fundraiser }]
})

// GET Updated DATA
mock.onGet('/api/fundraisers/list/data').reply(config => {
  const {
    q = '',
    page = 1,
    role = null,
  ...
  const dataAsc = data.fundraisers.sort((a, b) => (a[sortColumn] < b[sortColumn] ? -1 : 1))

  const dataToFilter = sort === 'asc' ? dataAsc : dataAsc.reverse()

  const filteredData = dataToFilter.filter(
    fundraiser =>
      (fundraiser.email.toLowerCase().includes(queryLowered) ||
      fundraiser.fullName.toLowerCase().includes(queryLowered) ||
      fundraiser.billing.toLowerCase().includes(queryLowered)) &&
      fundraiser.role === (role || fundraiser.role) &&
      fundraiser.currentPlan === (currentPlan || fundraiser.currentPlan) &&
  ...
  return [
    200,
    {
      ...
      fundraisers: paginateArray(filteredData, perPage, page)
    }
  ]
})

// GET USER
mock.onGet('/api/fundraisers/fundraiser').reply(config => {
  const { id } = config
  const fundraiser = data.fundraisers.find(i => i.id === id)
  return [200, { fundraiser }]
})

// DELETE: Deletes User
mock.onDelete('/apps/fundraisers/delete').reply(config => {
  // Get fundraiser id from URL
  let fundraiserId = config.id

  // Convert Id to number
  fundraiserId = Number(fundraiserId)

  const fundraiserIndex = data.fundraisers.findIndex(t => t.id === fundraiserId)
  data.fundraisers.splice(fundraiserIndex, 1)
